...
def confusion_graph():
    ...
    ################ -- OUTPUT -- ################    
    imgrutabase = "/Pseudomonas/WORK/ANALYSIS_CDHIT/MATRIXCDHIT/OUTPUTpy275GraphandResults/"
    imgic = "importancia_caracteristicas.png"
    imgic2 = "importancia_caracteristicas_log.png" #------------------------- // NUEVO \\ --------------------------#
    mtzconfusion = "confusion_matrix.csv"
    histo="histograma_tlabels.png" # Nombre del histograma si solo son dos nichos que se estan comparando #------------------------- // NUEVO \\ --------------------------#
    ...
    pd.DataFrame(class_etiquetas).drop_duplicates().to_csv(rutaetiquetas_codificadas, index=False) #------------------------- // NUEVO \\ --------------------------#
    #print(mtz_class_caracteres)
    #sys.stdout.flush()
    ...
    if len(zeroconteonichos.index) > 2:
        ...
        for i, cluster in enumerate(clusters):
            ...
            plt.bar(pos, cluster_data['Frecuencia'], width=bar_width, color=[colors[0],colors[1]])
